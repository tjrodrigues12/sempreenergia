...
function EWD_US_Meta_Box( $post ) {
	...
	$UPCP_Products = $items_table_name != '' ? $wpdb->get_results("SELECT Item_ID, Item_Name FROM $items_table_name") : array();
...
function EWD_US_Save_Meta_Box_Data($post_id) {
	...
	if ($_POST['content_type'] == 'upcp_product') {update_post_meta( $post_id, 'EWD_US_UPCP_Product_ID', sanitize_text_field( $_POST['upcp_products'] ) );}
	if ($_POST['content_type'] == 'woocommerce_product') {update_post_meta( $post_id, 'EWD_US_WC_Product_ID', sanitize_text_field( $_POST['wc_products'] ) );}
